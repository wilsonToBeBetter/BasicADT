...
/**
 * 二叉堆  实现优先级队列   最小二叉堆  堆序性和最大二叉堆相反
 * 最小二叉堆 原理同 最大二叉堆  不同的唯一之处  就是在比较函数less处做修改
 ...
 * 
 * 本MinPQ利用了  自下而上 自右而左的下滤 高效的实现了  public MinPQ(T[] a){//批量建立 二叉堆  heapification
 * @author wjs13
...
public class MinPQ<T extends Comparable<T>> {
    ...
    private int n ;//MinPQ最小二叉堆优先级队列中的元素的个数
    
    public void insert(T key){//按优先级顺序插入元素到MinPQ最小二叉堆   始终保证  完全二叉树的 结构性 和  堆序性
//        完全二叉堆 插入元素 就是 上滤  的过程
    ...
    public T delMin(){//找到MaxPQ并删除最小元素    采用二叉堆实现   即弹出根元素并删除 且始终使二叉堆的完全二叉树的结构性和堆序性保持
        //二叉堆 删除元素  就是   下滤  的过程
        if (n == 0) throw new NoSuchElementException("MaxPQ is underflow!");//下溢异常处理 增加健壮性
        T maxKey = (T) array[0];//最小二叉堆根结点 即为MaxPQ中最小的数   根结点也就是底层数组秩为0的元素
      ...
      //本while实现 根据 完全二叉树的性质    2 * i > n  则i为叶子结点  注意i为层序编号  不是数组的秩
        while (2 * i <= n){//在结点i有左孩子的情况下     即秩为i的结点不是叶子结点  且左孩子秩为2 * i
            ...
            if (2 * i + 1 <= n){//若结点i也有右孩子    2 * i + 1为右孩子秩
              //注意i和数组的秩相差一个1
               maxChild = less(array[2 * i - 1],array[2 * i + 1 - 1]) ? (2 * i + 1) : (2 * i) ; //找出左右孩子的较小的孩子的秩
//             即使两个孩子结点的值相同 结点i下滤到任何一个孩子结点的子树 都不会影响整体的堆序性  这点可以放心
    ...
    public T getMin(){//找到MaxPQ最小元素       采用二叉堆实现   找出根元素即可
        return (T) array[0];
    }
    ...
    private boolean less(Comparable key1, Comparable key2){
        return key2.compareTo(key1) < 0;//最小二叉堆 原理同 最大二叉堆  不同之处 就是在本比较函数做修改   compareTo key1和key2交换位置即可
    }
    ...
    public MinPQ(T[] a){//批量建立 二叉堆
        ...
        //自下而上 自右而左的 下滤  高效 实现 批量建立二叉堆   思想为delMin()中后半部分代码所做的尾结点替换根节点后进行的下滤 过程   也即子堆的逐层合并
        n = a.length;//利用的是下滤 一开始就可以设置二叉堆长度为数组a的长度
        array = a;//就地利用数组a进行 二叉堆的建立
      //只需要对数组a组成的逻辑完全二叉树的内部结点开始直到根节点 进行子堆的逐层合并
